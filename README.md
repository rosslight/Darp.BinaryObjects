...
- [ ] Blittable types
- [x] Enums
- [ ] Other .NET types: `BitArray` / `BigInteger`
...
The parsing step works the following way:

- Parse Attributes on field
- if IsArrayType?
    - GetUnderlyingTypeInfo
    - if Constant?
        - Get ElementLength from Attributes.ElementLength or UnderlyingTypeInfo
        - if Attributes.ElementCount or Attributes.FieldLength: `ConstantArraySymbol`
        - if Attributes.ElementCountName: `VariableCountArraySymbol`
        - if Attributes.FieldLengthName: `VariableLengthArraySymbol`
        - else: `ArraySymbol`
    - if BinaryObject?
        - if: Attributes.ElementCount: `ConstantCountBinaryObjectArraySymbol`
        - if: Attributes.ElementCountName: `VariableCountBinaryObjectArraySymbol`
        - else: `BinaryObjectArraySymbol`
- GetTypeInfo
- if Constant?
    - if Attributes.FieldLengthName: `VariableSymbol`
    - else: `ConstantSymbol`
- if BinaryObject: `BinaryObjectSymbol`
- else: Invalid field type

IsArrayType: Array with Rank 1 or ReadOnlyMemory<T> or List<T>

GetTypeInfo:
- if Enum: Check underlying type
- if primitive integer: Constant type
- if BinaryConstant: Constant type
- if BinaryObject attribute: Generated BinaryObject
- if implements IBinaryObject: BinaryObject
- ... additional checks for supported types
